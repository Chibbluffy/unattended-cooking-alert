...
Logging: WARNING and above only — written to LOG_FILE set in config.env.
         ...
         To debug actively, change LOG_FILE in config.env to a persistent path.
"""
...
try:
    ...
    print(f"CRITICAL: Failed to load config.env ({_config_path}): {e}", file=sys.stderr)
    raise
...
# the SD card. Change LOG_FILE in config.env to persist logs when debugging.
logging.basicConfig(
    filename=LOG_FILE,
...
def extract_temperature_array(raw_frame):
    ...
    temp_u16 = temp_raw.view(np.uint16).reshape((THERMAL_HEIGHT, RAW_WIDTH))
        ...
        while True:
            ...
            if not ret or raw_frame is None:
                ...
                if consecutive_failures >= MAX_CONSECUTIVE_FAILURES:
                    log.warning(
                        "Camera read failed %d times in a row — "
                        "check USB connection.",
                        consecutive_failures,
                    )
                    consecutive_failures = 0  # reset so we don't spam the log
                time.sleep(1)
                continue
